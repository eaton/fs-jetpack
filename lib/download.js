  ...
  return new Promise((resolve, reject) => {
    fetch(url).then((response) => {
      if (resp.ok && resp.body) {
        ...
        resolve(finished(Readable.fromWeb(body).pipe(stream)));
      } else {
        return reject(new Error(resp.statusText));
      }
    });
  });
};
...
const downloadSync = (path, url, options) => {
  let waitFlag = true;
  let timeout = Date.now() + 30*1000;
  let output = null;
  download.downloadAsync(path, url, options).then(result => {
    waitFlag = false;
    output = result; 
  });
